...
%         end
        ...
        responses(rowcol,rowcolcnt(rowcol),:,:)=X;     % size(responses)=[7,10,23,8]
        % responses(1)=rowcol=number of buttons=7
        % responses(2)=rowcolcnt(rowcol)=number of trials per run=10
...
[ y1,score1 ] = mySTDA_test1( test_ST,W1_op,W2_op,w_ST );     % size(y1)=[1,1], size(score1)=[1,7]

%% Idle and Control Discrimination
msI=weight2_max{1}(:,i_sbj);
msC=weight2_max{2}(:,i_sbj);
[~,pval_I] = vartest2(score1',msI);       % Two-sample t-test, size(pval_I)=[1,1]
[~,pval_C] = vartest2(score1',msC);       % Two-sample t-test, size(pval_C)=[1,1]
pval(1)=pval_I;
pval(2)=pval_C;         % size(pval)=[1,2]
[P,I]=max(pval);        % size(P,I)=[1,1]
